    ...
    public async postToSalesforce(): Promise<SalesforcePostResult> {
        ...
        if (!salesforceAccessInfo.result) {
            ...
            try {
                ...
                salesforcePostResult.result = res.data.result;
                salesforcePostResult.message = res.data?.message;
            }
            catch(e) {
                salesforcePostResult.result = false;
                salesforcePostResult.message = 'Post data to salesforce failed: ' + e.message + ' ' + JSON.stringify(e.response.data) + '(SessionId: ' + salesforceAccessInfo.accessToken + ')';
            }
        }
